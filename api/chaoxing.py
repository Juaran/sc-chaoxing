...
class Chaoxing:
    ...
    def scLogin(self):
        ...
        random_form = re.findall('(?<=random_form".value=).*?(?=>)', response.text)[0] 
        loginData = {
            'loginName': self.usernm,
        ...
        self.logger.debug("发送登录数据")
        resp = self.session.post(login_url, data={ "ciphertext": ciphertext })
        self.logger.debug("收到返回数据")
        if resp.cookies.get('User_Tocken'):
            # 1. 获取ticket
            ticket_url = 'https://www.scedu.com.cn/ThirdPortalService/login!getTicket.ac'
            ticket = self.session.get(ticket_url).text
            # 2. 带ticket请求信息
            teacher_info_url = 'https://scts.scedu.com.cn/Account/GetTeachInfo.do?ticket=' + ticket
            res = self.session.get(teacher_info_url)
            # 3. 请求主页
            ...
            chaoxing_url = re.findall("(?<=location.href=')(.*?)(?=')", self.session.get(hurl).text)[0]
            print("登录成功", nickName, chaoxing_url)
            self.uid = self.session.cookies['_uid']
            save_users(usernm=self.usernm, passwd=self.passwd)
            return True
        else:
            self.logger.error("登录失败：" + str(resp.json()))
